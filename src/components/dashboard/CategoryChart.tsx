      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="w-44 h-44 relative">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={categoryExpenses} cx="50%" cy="50%" innerRadius={50} outerRadius={72} paddingAngle={3} dataKey="value" stroke="none">
                {categoryExpenses.map((entry, i) => (
                  <Cell key={i} fill={entry.color} />
                ))}
              </Pie>
              <Tooltip formatter={(value: number) => formatValue(value)}
                contentStyle={{ background: "hsl(240 5% 9%)", border: "1px solid hsl(240 4% 20%)", borderRadius: "12px", color: "hsl(0 0% 95%)", fontSize: "12px" }} />
            </PieChart>
          </ResponsiveContainer>
