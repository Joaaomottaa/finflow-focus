...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    return transactions.filter((t) => {
      ...
      const matchFilter = filter === "Todos" ? true : filter === "Receitas" ? t.type === "income" : t.type === "expense";
    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
          ...
          <div className="flex gap-2 p-1 bg-card border border-border rounded-xl">
            {filterOptions.map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${filter === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                {f}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {filtered.map((t) => (
            <div key={t.id} className="glass-card p-4 flex items-center justify-between animate-fade-in">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${t.type === "income" ? "bg-income/10" : "bg-expense/10"}`}>
                  {t.type === "income" ? <ArrowDownLeft className="w-5 h-5 text-income" /> : <ArrowUpRight className="w-5 h-5 text-expense" />}
                </div>
                <div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground text-sm">Nenhuma transação encontrada.</div>
          )}
        </div>
      </div>
    </AppLayout>
